...
public class IpAddressUtils {
    ...
    private void initIp2regionResource() throws Exception {
        ...
        try {
            ...
            if (method == null) {
                log.error("未能找到 search 方法");
            }
        } catch (NoSuchMethodException e) {
            log.error("未找到 search 方法: {}", e.getMessage(), e);
        }
    }
    ...
    public static String getCityInfo(String ip) {
        ...
        try {
            ...
            log.error("IP解析反射调用异常: {}", targetException.getMessage(), targetException);
        } catch (IllegalAccessException e) {
            log.error("IP解析访问权限异常: {}", e.getMessage(), e);
        } catch (Exception e) {
            // 保留通用异常处理
            log.error("IP解析未知异常: {}", e.getMessage(), e);
        }
        return "";
    }
